...
import com.kuibu.algorithm.find.entity.State;
...
public class AhoCorasickDoubleArrayTrie<V> implements Serializable {
    ...
    private static List<Map<String, String>> imageShieldStruct(List<AhoCorasickDoubleArrayTrie<String>.Hit<String>> imageTitleHits) {
        //遍历所有的屏蔽词，并添加到list中，返回进行提示
        List<Map<String, String>> imageShieldList = new ArrayList<Map<String, String>>();
        Map<String, String> contentSheildMap = null;
        for (AhoCorasickDoubleArrayTrie<String>.Hit<String> hit : imageTitleHits) {
            //shield：屏蔽词内容		begin：屏蔽词在文本中的起始位置	end:屏蔽词在文本中的结束位置
            contentSheildMap = new HashMap<String, String>();
            contentSheildMap.put("shield", hit.value);
            contentSheildMap.put("begin", hit.begin + 1 + "");
            contentSheildMap.put("end", hit.end + "");
            imageShieldList.add(contentSheildMap);
        }
        return imageShieldList;
    }
    ...
    public static void main(String[] args) {
        ...
        for (String str : images) {
            ...
            List<AhoCorasickDoubleArrayTrie<String>.Hit<String>> imageTitleHits = arrayTrie.parseText(str);
            if (imageTitleHits.size() > 0) {
                List<Map<String, String>> imageShieldList = imageShieldStruct(imageTitleHits);
                resultMap.put("url", "url" + i);
                resultMap.put("shieldContent", imageShieldList);
